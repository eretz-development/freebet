		...
		if($_POST['resultat'] != "null"){
			if($_POST['mise'] >= '100'){
				if(($stackuser['stack']-$_POST['mise']) >= 0){
					$a = $db->prepare("SELECT equipe_une,equipe_deux,cote_equipe_une,cote_nul,cote_equipe_deux FROM game_with_equality WHERE Id = :Id");
					...
					foreach($answer as $ans){
						...
						} elseif ($_POST['resultat'] == $ans['equipe_deux']) {
							$cote_mise = $ans['cote_equipe_deux'];
							$perdant = $ans['equipe_une'];
						} else {
							$cote_mise = $ans['cote_nul'];
							$perdant = $ans['equipe_deux'];
						}
					}
